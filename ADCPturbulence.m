clc; clear; close all
load('S100882A003_LakeK_Sig2_8.mat');
...
for i = 1:bins
    ...
    % power spectra (bins are in rows, 8Hz hour long ensembles in columns)
    if i==63
        for j=1:ensembles
            [psd_w(:,j), f(:,j)] = pwelch(w_prime(:,j),[],[],[],8);
        end
        figure
        loglog(f, psd_w);
        xlabel('Frequency (Hz)');
        ylabel('PSD');
        title('w dir, z = 2.64m (bin 63)');
    end
